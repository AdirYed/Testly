  ...
  <div>
    ...
    >
      ...
      <template v-if="!isLoading">
        <vue-headful
          description="טסטלי מבחני תאוריה. האתר מספק סימולציית מבחני תאוריה לכל הרישיונות באופן חדשני, מקצועי, איכותי וחינמי! תוצאות מבחנים בטסטלי."
          title="טסטלי - מבחני תאוריה - תוצאות מבחנים"
        />
              ...
              <ul class="tw-flex tw-flex-wrap tw-flex-col tw--my-1 tw-mt-1">
                ...
                >
                  <label
                    :class="{
                      'tw-bg-primary':
                        quiz.test_report_answers[currQuestion - 1][
                          'answer_id'
                        ] === response.id,
                      // score
                ...
                <button
                  :class="{
                    'tw-border-primary tw-bg-primary':
                      i !== currQuestion &&
                      quiz.test_report_answers[n]['answer_id'] !== null,
                    'tw-text-green-700 tw-font-bold':
                      rightAnswer(n) ===
                      quiz.test_report_answers[n]['answer_id'],
                    'tw-text-red-500 tw-font-bold':
                      rightAnswer(n) !==
                        quiz.test_report_answers[n]['answer_id'] &&
                      quiz.test_report_answers[n]['answer_id'] !== null,
                  ...
                  style="width: 45%"
                  v-for="(i, n) in quiz.test_report_answers.length"
                >
                  שאלה {{ i }}
                </button>
              </div>
      ...
      <div class="tw-pt-5" v-else-if="error === 'uuid'">
        <vue-headful
...
export default {
  ...
  methods: {
    ...
    rightAnswer(question) {
      for (let i = 0; i < 4; i++) {
        if (
          this.quiz.test_report_answers[question]["question"]["answers"][i][
            "is_correct"
          ]
        ) {
          return this.quiz.test_report_answers[question]["question"]["answers"][
            i
          ]["id"];
        }
      }
    ...
    rightAnswers() {
      ...
      for (let i = 0; i < this.quiz.test_report_answers.length; i++) {
        if (
          ...
          this.rightAnswer(i) === this.quiz.test_report_answers[i]["answer_id"]
        ) {
          rightAnswers++;
        }
      }
  ...
  computed: {
    ...
    question() {
      return this.quiz.test_report_answers[this.questionIndex]["question"];
    },
